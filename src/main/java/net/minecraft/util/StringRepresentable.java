   ...
   static Keyable keys(final StringRepresentable[] pSerializables) {
      return new Keyable() {
         public <T> Stream<T> keys(DynamicOps<T> p_14401_) {
            return p_14401_.compressMaps() ? IntStream.range(0, pSerializables.length).mapToObj(p_14401_::createInt) : Arrays.stream(pSerializables).map(StringRepresentable::getSerializedName).map(p_14401_::createString);
         }
      };
   }
}
