...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
   ...
   private void scheduleUnload(long pChunkPos, ChunkHolder pChunkHolder) {
      ...
      completablefuture.thenAcceptAsync((p_140309_) -> {
         ...
         if (completablefuture1 != completablefuture) {
            ...
            if (this.pendingUnloads.remove(pChunkPos, pChunkHolder) && p_140309_ != null) {
               if (p_140309_ instanceof LevelChunk) {
                  ((LevelChunk)p_140309_).setLoaded(false);
               }

               this.save(p_140309_);
               if (this.entitiesInLevel.remove(pChunkPos) && p_140309_ instanceof LevelChunk) {
         ...
         if (p_140304_ != null) {
            LOGGER.error("Failed to save chunk {}", pChunkHolder.getPos(), p_140304_);
         }
   ...
   private CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> scheduleChunkGeneration(ChunkHolder pChunkHolder, ChunkStatus pChunkStatus) {
      ...
      CompletableFuture<Either<List<ChunkAccess>, ChunkHolder.ChunkLoadingFailure>> completablefuture = this.getChunkRangeFuture(chunkpos, pChunkStatus.getRange(), (p_140346_) -> {
         return this.getDependencyStatus(pChunkStatus, p_140346_);
      });
      this.level.getProfiler().incrementCounter(() -> {
         return "chunkGenerate " + pChunkStatus.getName();
      });
      Executor executor = (p_143148_) -> {
         this.worldgenMailbox.tell(ChunkTaskPriorityQueueSorter.message(pChunkHolder, p_143148_));
      };
      return completablefuture.thenComposeAsync((p_143083_) -> {
         return p_143083_.map((p_143089_) -> {
            try {
               CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> completablefuture1 = pChunkStatus.generate(executor, this.level, this.generator, this.structureManager, this.lightEngine, (p_143113_) -> {
   ...
   protected void updateChunkTracking(ServerPlayer pPlayer, ChunkPos pChunkPos, Packet<?>[] pPacketCache, boolean pWasLoaded, boolean pLoad) {
   ...
   boolean noPlayersCloseForSpawning(ChunkPos pChunkPos) {
      ...
      return !this.distanceManager.hasPlayersNearby(i) ? true : this.playerMap.getPlayers(i).noneMatch((p_140221_) -> {
         return !p_140221_.isSpectator() && euclideanDistanceSquared(pChunkPos, p_140221_) < 16384.0D;
      });
   }
   ...
   void updatePlayerStatus(ServerPlayer pPlayer, boolean pTrack) {
      ...
      for(int l = i - this.viewDistance; l <= i + this.viewDistance; ++l) {
         for(int k = j - this.viewDistance; k <= j + this.viewDistance; ++k) {
            ...
            this.updateChunkTracking(pPlayer, chunkpos, new Packet[2], !pTrack, pTrack);
         }
      }
      ...
      if (Math.abs(k - l1) <= this.viewDistance * 2 && Math.abs(l - i2) <= this.viewDistance * 2) {
         ...
         for(int l3 = k2; l3 <= j3; ++l3) {
            for(int k1 = i3; k1 <= k3; ++k1) {
               ...
               this.updateChunkTracking(pPlayer, chunkpos1, new Packet[2], flag5, flag6);
            }
         }
      } else {
         for(int i1 = k - this.viewDistance; i1 <= k + this.viewDistance; ++i1) {
            for(int j1 = l - this.viewDistance; j1 <= l + this.viewDistance; ++j1) {
               ...
               boolean flag3 = true;
               boolean flag4 = false;
               this.updateChunkTracking(pPlayer, chunkpos, new Packet[2], true, false);
            }
         }
         ...
         for(int j2 = l1 - this.viewDistance; j2 <= l1 + this.viewDistance; ++j2) {
            for(int l2 = i2 - this.viewDistance; l2 <= i2 + this.viewDistance; ++l2) {
               ...
               boolean flag7 = false;
               boolean flag8 = true;
               this.updateChunkTracking(pPlayer, chunkpos2, new Packet[2], false, true);
            }
         }
      }
   ...
   public Stream<ServerPlayer> getPlayers(ChunkPos pPos, boolean pBoundaryOnly) {
      return this.playerMap.getPlayers(pPos.toLong()).filter((p_140257_) -> {
         int i = checkerboardDistance(pPos, p_140257_, true);
         if (i > this.viewDistance) {
            return false;
         } else {
   ...
   protected void addEntity(Entity pEntity) {
      if (!(pEntity instanceof EnderDragonPart)) {
   ...
   private void playerLoadedChunk(ServerPlayer pPlayer, Packet<?>[] pPacketCache, LevelChunk pChunk) {
      if (pPacketCache[0] == null) {
         pPacketCache[0] = new ClientboundLevelChunkPacket(pChunk);
         pPacketCache[1] = new ClientboundLightUpdatePacket(pChunk.getPos(), this.lightEngine, (BitSet)null, (BitSet)null, true);
      }
