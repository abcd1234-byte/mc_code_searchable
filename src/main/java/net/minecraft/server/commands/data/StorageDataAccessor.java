...
public class StorageDataAccessor implements DataAccessor {
   ...
   public Component getPrintSuccess(Tag pNbt) {
      return new TranslatableComponent("commands.data.storage.query", this.id, NbtUtils.toPrettyComponent(pNbt));
   }

   public Component getPrintSuccess(NbtPathArgument.NbtPath pPath, double pScale, int pValue) {
      return new TranslatableComponent("commands.data.storage.get", pPath, this.id, String.format(Locale.ROOT, "%.2f", pScale), pValue);
   }
}