...
public class EntityDataAccessor implements DataAccessor {
   ...
   public Component getPrintSuccess(Tag pNbt) {
      return new TranslatableComponent("commands.data.entity.query", this.entity.getDisplayName(), NbtUtils.toPrettyComponent(pNbt));
   }

   public Component getPrintSuccess(NbtPathArgument.NbtPath pPath, double pScale, int pValue) {
      return new TranslatableComponent("commands.data.entity.get", pPath, this.entity.getDisplayName(), String.format(Locale.ROOT, "%.2f", pScale), pValue);
   }
}