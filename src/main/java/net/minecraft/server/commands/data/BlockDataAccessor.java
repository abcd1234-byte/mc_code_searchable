...
public class BlockDataAccessor implements DataAccessor {
   ...
   public Component getPrintSuccess(Tag pNbt) {
      return new TranslatableComponent("commands.data.block.query", this.pos.getX(), this.pos.getY(), this.pos.getZ(), NbtUtils.toPrettyComponent(pNbt));
   }

   public Component getPrintSuccess(NbtPathArgument.NbtPath pPath, double pScale, int pValue) {
      return new TranslatableComponent("commands.data.block.get", pPath, this.pos.getX(), this.pos.getY(), this.pos.getZ(), String.format(Locale.ROOT, "%.2f", pScale), pValue);
   }
}