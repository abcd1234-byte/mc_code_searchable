...
public class GameTestServer extends MinecraftServer {
   ...
   private GameTestServer(Thread pServerThread, LevelStorageSource.LevelStorageAccess pStorageSource, PackRepository pPackRepository, ServerResources pResources, Collection<GameTestBatch> pTestBatches, BlockPos pSpawnPos, RegistryAccess.RegistryHolder pRegistryHolder, Registry<Biome> pBiomeRegistry, Registry<DimensionType> pDimensionTypeRegistry) {
      super(pServerThread, pRegistryHolder, pStorageSource, new PrimaryLevelData(TEST_SETTINGS, new WorldGenSettings(0L, false, false, WorldGenSettings.withOverworld(pDimensionTypeRegistry, DimensionType.defaultDimensions(pDimensionTypeRegistry, pBiomeRegistry, pRegistryHolder.registryOrThrow(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY), 0L), new FlatLevelSource(FlatLevelGeneratorSettings.getDefault(pBiomeRegistry)))), Lifecycle.stable()), pPackRepository, Proxy.NO_PROXY, DataFixers.getDataFixer(), pResources, (MinecraftSessionService)null, (GameProfileRepository)null, (GameProfileCache)null, LoggerChunkProgressListener::new);
   ...
   private void startTests(ServerLevel pServerLevel) {
      Collection<GameTestInfo> collection = GameTestRunner.runTestBatches(this.testBatches, new BlockPos(0, 4, 0), Rotation.NONE, pServerLevel, GameTestTicker.SINGLETON, 8);
