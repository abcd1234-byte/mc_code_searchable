...
public abstract class Registry<T> implements Codec<T>, Keyable, IdMap<T> {
   ...
   public static <T extends WritableRegistry<?>> void checkRegistry(WritableRegistry<T> pMetaRegistry) {
      pMetaRegistry.forEach((p_175461_) -> {
         if (p_175461_.keySet().isEmpty()) {
            Util.logAndPauseIfInIde("Registry '" + pMetaRegistry.getKey(p_175461_) + "' was empty after loading");
         }
   ...
   private static <T> Registry<T> registerSimple(ResourceKey<? extends Registry<T>> pRegistryKey, Supplier<T> pLoader) {
      return registerSimple(pRegistryKey, Lifecycle.experimental(), pLoader);
   }

   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> pRegistryKey, String pDefaultValueName, Supplier<T> pLoader) {
      return registerDefaulted(pRegistryKey, pDefaultValueName, Lifecycle.experimental(), pLoader);
   }

   private static <T> Registry<T> registerSimple(ResourceKey<? extends Registry<T>> pRegistryKey, Lifecycle pLifecycle, Supplier<T> pLoader) {
      return internalRegister(pRegistryKey, new MappedRegistry<>(pRegistryKey, pLifecycle), pLoader, pLifecycle);
   }

   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> pRegistryKey, String pDefaultValueName, Lifecycle pLifecycle, Supplier<T> pLoader) {
      return internalRegister(pRegistryKey, new DefaultedRegistry<>(pDefaultValueName, pRegistryKey, pLifecycle), pLoader, pLifecycle);
   }

   private static <T, R extends WritableRegistry<T>> R internalRegister(ResourceKey<? extends Registry<T>> pRegistryKey, R pRegistry, Supplier<T> pLoader, Lifecycle pLifecycle) {
   ...
   protected abstract Lifecycle lifecycle(T pValue);
   ...
   public static <V, T extends V> T register(Registry<V> pRegistry, ResourceLocation pName, T pValue) {
      return (T)((WritableRegistry)pRegistry).register(ResourceKey.create(pRegistry.key, pName), pValue, Lifecycle.stable());
   }

   public static <V, T extends V> T registerMapping(Registry<V> pRegistry, int pId, String pName, T pValue) {
      return (T)((WritableRegistry)pRegistry).registerMapping(pId, ResourceKey.create(pRegistry.key, new ResourceLocation(pName)), pValue, Lifecycle.stable());
   }
