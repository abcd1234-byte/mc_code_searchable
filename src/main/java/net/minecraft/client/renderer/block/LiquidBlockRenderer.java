...
public class LiquidBlockRenderer {
   ...
   public boolean tesselate(BlockAndTintGetter pLevel, BlockPos pPos, VertexConsumer pConsumer, FluidState pFluidState) {
      ...
      int i = flag ? 16777215 : BiomeColors.getAverageWaterColor(pLevel, pPos);
      ...
      if (!flag1 && !flag2 && !flag6 && !flag5 && !flag3 && !flag4) {
         ...
         if (flag1 && !isFaceOccludedByNeighbor(pLevel, pPos, Direction.UP, Math.min(Math.min(f7, f8), Math.min(f9, f10)))) {
            ...
            float f26 = f4 * f1;
            float f27 = f4 * f2;
            this.vertex(pConsumer, d0 + 0.0D, d1 + (double)f7, d2 + 0.0D, f25, f26, f27, f13, f17, j);
            this.vertex(pConsumer, d0 + 0.0D, d1 + (double)f8, d2 + 1.0D, f25, f26, f27, f14, f18, j);
            this.vertex(pConsumer, d0 + 1.0D, d1 + (double)f9, d2 + 1.0D, f25, f26, f27, f15, f19, j);
            this.vertex(pConsumer, d0 + 1.0D, d1 + (double)f10, d2 + 0.0D, f25, f26, f27, f16, f20, j);
         ...
         if (flag2) {
            ...
            float f42 = f3 * f1;
            float f43 = f3 * f2;
            this.vertex(pConsumer, d0, d1 + (double)f12, d2 + 1.0D, f41, f42, f43, f35, f39, i1);
            this.vertex(pConsumer, d0, d1 + (double)f12, d2, f41, f42, f43, f35, f37, i1);
            this.vertex(pConsumer, d0 + 1.0D, d1 + (double)f12, d2, f41, f42, f43, f36, f37, i1);
            this.vertex(pConsumer, d0 + 1.0D, d1 + (double)f12, d2 + 1.0D, f41, f42, f43, f36, f39, i1);
            flag7 = true;
         }
         ...
         for(int l = 0; l < 4; ++l) {
            ...
            if (flag8 && !isFaceOccludedByNeighbor(pLevel, pPos, direction, Math.max(f38, f40))) {
               ...
               float f33 = f4 * f31 * f1;
               float f34 = f4 * f31 * f2;
               this.vertex(pConsumer, d3, d1 + (double)f38, d4, f32, f33, f34, f49, f28, k);
               this.vertex(pConsumer, d5, d1 + (double)f40, d6, f32, f33, f34, f50, f29, k);
               this.vertex(pConsumer, d5, d1 + (double)f12, d6, f32, f33, f34, f50, f30, k);
               this.vertex(pConsumer, d3, d1 + (double)f12, d4, f32, f33, f34, f49, f30, k);
