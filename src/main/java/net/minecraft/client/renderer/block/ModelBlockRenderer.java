...
public class ModelBlockRenderer {
   ...
   public boolean tesselateBlock(BlockAndTintGetter pLevel, BakedModel pModel, BlockState pState, BlockPos pPos, PoseStack pPoseStack, VertexConsumer pConsumer, boolean pCheckSides, Random pRandom, long pSeed, int pPackedOverlay) {
      boolean flag = Minecraft.useAmbientOcclusion() && pState.getLightEmission() == 0 && pModel.useAmbientOcclusion();
      Vec3 vec3 = pState.getOffset(pLevel, pPos);
      pPoseStack.translate(vec3.x, vec3.y, vec3.z);

      try {
         return flag ? this.tesselateWithAO(pLevel, pModel, pState, pPos, pPoseStack, pConsumer, pCheckSides, pRandom, pSeed, pPackedOverlay) : this.tesselateWithoutAO(pLevel, pModel, pState, pPos, pPoseStack, pConsumer, pCheckSides, pRandom, pSeed, pPackedOverlay);
   ...
   public boolean tesselateWithAO(BlockAndTintGetter pLevel, BakedModel pModel, BlockState pState, BlockPos pPos, PoseStack pPoseStack, VertexConsumer pConsumer, boolean pCheckSides, Random pRandom, long pSeed, int pPackedOverlay) {
      boolean flag = false;
      ...
      for(Direction direction : DIRECTIONS) {
         ...
         if (!list.isEmpty()) {
            ...
            if (!pCheckSides || Block.shouldRenderFace(pState, pLevel, pPos, direction, blockpos$mutableblockpos)) {
               this.renderModelFaceAO(pLevel, pState, pPos, pPoseStack, pConsumer, list, afloat, bitset, modelblockrenderer$ambientocclusionface, pPackedOverlay);
               flag = true;
            }
         }
      }
      ...
      pRandom.setSeed(pSeed);
      List<BakedQuad> list1 = pModel.getQuads(pState, (Direction)null, pRandom);
      if (!list1.isEmpty()) {
         this.renderModelFaceAO(pLevel, pState, pPos, pPoseStack, pConsumer, list1, afloat, bitset, modelblockrenderer$ambientocclusionface, pPackedOverlay);
         flag = true;
      }
   ...
   public boolean tesselateWithoutAO(BlockAndTintGetter pLevel, BakedModel pModel, BlockState pState, BlockPos pPos, PoseStack pPoseStack, VertexConsumer pConsumer, boolean pCheckSides, Random pRandom, long pSeed, int pPackedOverlay) {
      boolean flag = false;
      BitSet bitset = new BitSet(3);
      BlockPos.MutableBlockPos blockpos$mutableblockpos = pPos.mutable();
   ...
   public void renderModel(PoseStack.Pose pPose, VertexConsumer pConsumer, @Nullable BlockState pState, BakedModel pModel, float pRed, float pGreen, float pBlue, int pPackedLight, int pPackedOverlay) {
      ...
      for(Direction direction : DIRECTIONS) {
         random.setSeed(42L);
         renderQuadList(pPose, pConsumer, pRed, pGreen, pBlue, pModel.getQuads(pState, direction, random), pPackedLight, pPackedOverlay);
      }

      random.setSeed(42L);
      renderQuadList(pPose, pConsumer, pRed, pGreen, pBlue, pModel.getQuads(pState, (Direction)null, random), pPackedLight, pPackedOverlay);
   }
