...
public class PostChain implements AutoCloseable {
   ...
   private void load(TextureManager pTextureManager, ResourceLocation pResourceLocation) throws IOException, JsonSyntaxException {
      ...
      try {
         resource = this.resourceManager.getResource(pResourceLocation);
         ...
         chainedjsonexception.setFilenameAndFlush(pResourceLocation.getPath() + s);
   ...
   private RenderTarget getRenderTarget(String pTarget) {
      if (pTarget == null) {
         return null;
      } else {
