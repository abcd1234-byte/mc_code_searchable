   ...
   public void move(double pX, double pY, double pZ) {
      if (!this.stoppedByCollision) {
         ...
         if (this.hasPhysics && (pX != 0.0D || pY != 0.0D || pZ != 0.0D)) {
            Vec3 vec3 = Entity.collideBoundingBoxHeuristically((Entity)null, new Vec3(pX, pY, pZ), this.getBoundingBox(), this.level, CollisionContext.empty(), new RewindableStream<>(Stream.empty()));
            pX = vec3.x;
            pY = vec3.y;
            pZ = vec3.z;
         }
