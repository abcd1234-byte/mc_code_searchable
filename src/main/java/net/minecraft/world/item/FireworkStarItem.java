...
public class FireworkStarItem extends Item {
   ...
   public static void appendHoverText(CompoundTag pCompound, List<Component> pTooltipComponents) {
      ...
      if (aint.length > 0) {
         pTooltipComponents.add(appendColors((new TextComponent("")).withStyle(ChatFormatting.GRAY), aint));
      }

      int[] aint1 = pCompound.getIntArray("FadeColors");
      if (aint1.length > 0) {
         pTooltipComponents.add(appendColors((new TranslatableComponent("item.minecraft.firework_star.fade_to")).append(" ").withStyle(ChatFormatting.GRAY), aint1));
      }
