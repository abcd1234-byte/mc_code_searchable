         ...
         if (compoundtag != null && compoundtag.contains("BucketVariantTag", 3)) {
            ...
            for(int j = 0; j < TropicalFish.COMMON_VARIANTS.length; ++j) {
               if (i == TropicalFish.COMMON_VARIANTS[j]) {
                  pTooltipComponents.add((new TranslatableComponent(TropicalFish.getPredefinedName(j))).withStyle(achatformatting));
                  return;
               }
            }

            pTooltipComponents.add((new TranslatableComponent(TropicalFish.getFishTypeName(i))).withStyle(achatformatting));
