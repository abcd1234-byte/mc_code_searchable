...
public final class Biome {
   ...
   public boolean isColdEnoughToSnow(BlockPos pPos) {
   ...
   public void generate(StructureFeatureManager pStructureManager, ChunkGenerator pChunkGenerator, WorldGenRegion pLevel, long pSeed, WorldgenRandom pRandom, BlockPos pPos) {
      List<List<Supplier<ConfiguredFeature<?, ?>>>> list = this.generationSettings.features();
      Registry<ConfiguredFeature<?, ?>> registry = pLevel.registryAccess().registryOrThrow(Registry.CONFIGURED_FEATURE_REGISTRY);
      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (pStructureManager.shouldGenerateFeatures()) {
            for(StructureFeature<?> structurefeature : this.structuresByStep.getOrDefault(j, Collections.emptyList())) {
               ...
               int l = SectionPos.blockToSectionCoord(pPos.getX());
               int i1 = SectionPos.blockToSectionCoord(pPos.getZ());
               ...
               try {
                  int l1 = pLevel.getMinBuildHeight() + 1;
                  int i2 = pLevel.getMaxBuildHeight() - 1;
                  pLevel.setCurrentlyGenerating(supplier);
                  pStructureManager.startsForFeature(SectionPos.of(pPos), structurefeature).forEach((p_151667_) -> {
                     p_151667_.placeInChunk(pLevel, pStructureManager, pChunkGenerator, pRandom, new BoundingBox(j1, l1, k1, j1 + 15, i2, k1 + 15), new ChunkPos(l, i1));
                  });
         ...
         if (list.size() > j) {
            for(Supplier<ConfiguredFeature<?, ?>> supplier1 : list.get(j)) {
               ...
               try {
                  pLevel.setCurrentlyGenerating(supplier2);
                  configuredfeature.place(pLevel, pChunkGenerator, pRandom, pPos);
   ...
   public void buildSurfaceAt(Random pRandom, ChunkAccess pChunk, int pX, int pZ, int pHeight, double pNoise, BlockState pDefaultBlock, BlockState pDefaultFluid, int pSeaLevel, int pMinSurfaceLevel, long pSeed) {
      ConfiguredSurfaceBuilder<?> configuredsurfacebuilder = this.generationSettings.getSurfaceBuilder().get();
      configuredsurfacebuilder.initNoise(pSeed);
      configuredsurfacebuilder.apply(pRandom, pChunk, this, pX, pZ, pHeight, pNoise, pDefaultBlock, pDefaultFluid, pSeaLevel, pMinSurfaceLevel, pSeed);
   }
   ...
   public static class ClimateParameters {
      ...
      public boolean equals(Object pOther) {
         if (this == pOther) {
            ...
            Biome.ClimateParameters biome$climateparameters = (Biome.ClimateParameters)pOther;
