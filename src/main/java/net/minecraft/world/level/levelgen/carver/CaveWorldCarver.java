...
public class CaveWorldCarver extends WorldCarver<CaveCarverConfiguration> {
   ...
   public boolean carve(CarvingContext pContext, CaveCarverConfiguration pConfig, ChunkAccess pChunk, Function<BlockPos, Biome> pBiomeAccessor, Random pRandom, Aquifer pAquifer, ChunkPos pChunkPos, BitSet pCarvingMask) {
      ...
      for(int k = 0; k < j; ++k) {
         ...
         if (pRandom.nextInt(4) == 0) {
            ...
            this.createRoom(pContext, pConfig, pChunk, pBiomeAccessor, pRandom.nextLong(), pAquifer, d0, d1, d2, f1, d6, pCarvingMask, worldcarver$carveskipchecker);
            l += pRandom.nextInt(4);
         }
   ...
   protected void createRoom(CarvingContext pContext, CaveCarverConfiguration pConfig, ChunkAccess pChunk, Function<BlockPos, Biome> pBiomeAccessor, long pSeed, Aquifer pAquifer, double pX, double pY, double pZ, float pRadius, double pHorizontalVerticalRatio, BitSet pCarvingMask, WorldCarver.CarveSkipChecker pSkipChecker) {
      double d0 = 1.5D + (double)(Mth.sin(((float)Math.PI / 2F)) * pRadius);
      double d1 = d0 * pHorizontalVerticalRatio;
      this.carveEllipsoid(pContext, pConfig, pChunk, pBiomeAccessor, pSeed, pAquifer, pX + 1.0D, pY, pZ, d0, d1, pCarvingMask, pSkipChecker);
   }

   protected void createTunnel(CarvingContext pContext, CaveCarverConfiguration pConfig, ChunkAccess pChunk, Function<BlockPos, Biome> pBiomeAccessor, long pSeed, Aquifer pAquifer, double pX, double pY, double pZ, double pHorizontalRadiusMultiplier, double pVerticalRadiusMultiplier, float pThickness, float pYaw, float pPitch, int pBranchIndex, int pBranchCount, double pHorizontalVerticalRatio, BitSet pCarvingMask, WorldCarver.CarveSkipChecker pSkipChecker) {
      Random random = new Random(pSeed);
      int i = random.nextInt(pBranchCount / 2) + pBranchCount / 4;
      ...
      for(int j = pBranchIndex; j < pBranchCount; ++j) {
         ...
         pPitch = pPitch * (flag ? 0.92F : 0.7F);
         pPitch = pPitch + f1 * 0.1F;
         pYaw += f * 0.1F;
         f1 = f1 * 0.9F;
         f = f * 0.75F;
         ...
         if (j == i && pThickness > 1.0F) {
            this.createTunnel(pContext, pConfig, pChunk, pBiomeAccessor, random.nextLong(), pAquifer, pX, pY, pZ, pHorizontalRadiusMultiplier, pVerticalRadiusMultiplier, random.nextFloat() * 0.5F + 0.5F, pYaw - ((float)Math.PI / 2F), pPitch / 3.0F, j, pBranchCount, 1.0D, pCarvingMask, pSkipChecker);
            this.createTunnel(pContext, pConfig, pChunk, pBiomeAccessor, random.nextLong(), pAquifer, pX, pY, pZ, pHorizontalRadiusMultiplier, pVerticalRadiusMultiplier, random.nextFloat() * 0.5F + 0.5F, pYaw + ((float)Math.PI / 2F), pPitch / 3.0F, j, pBranchCount, 1.0D, pCarvingMask, pSkipChecker);
            return;
         }
         ...
         if (random.nextInt(4) != 0) {
            ...
            this.carveEllipsoid(pContext, pConfig, pChunk, pBiomeAccessor, pSeed, pAquifer, pX, pY, pZ, d0 * pHorizontalRadiusMultiplier, d1 * pVerticalRadiusMultiplier, pCarvingMask, pSkipChecker);
         }
      }
