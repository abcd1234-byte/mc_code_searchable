...
public class DripstoneClusterFeature extends Feature<DripstoneClusterConfiguration> {
   ...
   private void placeColumn(WorldGenLevel pLevel, Random pRandom, BlockPos pPos, int pX, int pZ, float pWetness, double pChance, int pHeight, float pDensity, DripstoneClusterConfiguration pConfig) {
      Optional<Column> optional = Column.scan(pLevel, pPos, pConfig.floorToCeilingSearchRange, DripstoneUtils::isEmptyOrWater, DripstoneUtils::isDripstoneBaseOrLava);
   ...
   private double getChanceOfStalagmiteOrStalactite(int pXRadius, int pZRadius, int pX, int pZ, DripstoneClusterConfiguration pConfig) {
      ...
      int k = Math.min(i, j);
      return Mth.clampedMap((double)k, 0.0D, (double)pConfig.maxDistanceFromEdgeAffectingChanceOfDripstoneColumn, (double)pConfig.chanceOfDripstoneColumnAtMaxDistanceFromCenter, 1.0D);
   }
