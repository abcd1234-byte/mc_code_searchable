...
public class ChunkTickList<T> implements TickList<T> {
   ...
   public ChunkTickList(Function<T, ResourceLocation> pToId, List<TickNextTickData<T>> pTicks, long pGameTime) {
      this(pToId, pTicks.stream().map((p_45642_) -> {
         return new ChunkTickList.ScheduledTick<>(p_45642_.getType(), p_45642_.pos, (int)(p_45642_.triggerTick - pGameTime), p_45642_.priority);
      }).collect(Collectors.toList()));
   }

   private ChunkTickList(Function<T, ResourceLocation> pToId, List<ChunkTickList.ScheduledTick<T>> pTicks) {
      this.ticks = pTicks;
      this.toId = pToId;
   }
   ...
   public void copyOut(TickList<T> pTickList) {
      this.ticks.forEach((p_45647_) -> {
         pTickList.scheduleTick(p_45647_.pos, p_45647_.type, p_45647_.delay, p_45647_.priority);
      });
   }
   ...
   static class ScheduledTick<T> {
      ...
      ScheduledTick(T pType, BlockPos pPos, int pDelay, TickPriority pPriority) {
         this.type = pType;
         this.pos = pPos;
         this.delay = pDelay;
