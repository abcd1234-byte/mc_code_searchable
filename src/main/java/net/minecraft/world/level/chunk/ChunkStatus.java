...
public class ChunkStatus {
   ...
   ChunkStatus(String pName, @Nullable ChunkStatus pParent, int pRange, EnumSet<Heightmap.Types> pHeightmapsAfter, ChunkStatus.ChunkType pChunkType, ChunkStatus.GenerationTask pGenerationTask, ChunkStatus.LoadingTask pLoadingTask) {
      this.name = pName;
   ...
   public CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> generate(Executor pExecutor, ServerLevel pLevel, ChunkGenerator pGenerator, StructureManager pStructureManager, ThreadedLevelLightEngine pLightEngine, Function<ChunkAccess, CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>> pTask, List<ChunkAccess> pLoadingChunk) {
      return this.generationTask.doWork(this, pExecutor, pLevel, pGenerator, pStructureManager, pLightEngine, pTask, pLoadingChunk, pLoadingChunk.get(pLoadingChunk.size() / 2));
   }
   ...
   interface GenerationTask {
      CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> doWork(ChunkStatus pStatus, Executor pExecutor, ServerLevel pLevel, ChunkGenerator pGenerator, StructureManager pStructureManager, ThreadedLevelLightEngine pLightEngine, Function<ChunkAccess, CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>> pTask, List<ChunkAccess> pNeighboringChunks, ChunkAccess pLoadingChunk);
   }
   ...
   interface SimpleGenerationTask extends ChunkStatus.GenerationTask {
      default CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> doWork(ChunkStatus pStatus, Executor pExecutor, ServerLevel pLevel, ChunkGenerator pGenerator, StructureManager pStructureManager, ThreadedLevelLightEngine pLightEngine, Function<ChunkAccess, CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>> pTask, List<ChunkAccess> pNeighboringChunks, ChunkAccess pLoadingChunk) {
         if (!pLoadingChunk.getStatus().isOrAfter(pStatus)) {
            this.doWork(pStatus, pLevel, pGenerator, pNeighboringChunks, pLoadingChunk);
            if (pLoadingChunk instanceof ProtoChunk) {
               ((ProtoChunk)pLoadingChunk).setStatus(pStatus);
            }
         }
