...
public class RealmsMainScreen extends RealmsScreen {
   ...
   private boolean shouldPlayButtonBeActive(@Nullable RealmsServer pRealmsServer) {
   ...
   @Nullable
   RealmsServer findServer(long pServerId) {
      for(RealmsServer realmsserver : this.realmsServers) {
         if (realmsserver.id == pServerId) {
            return realmsserver;
         }
      }
   ...
   void removeServer(RealmsServer pRealmsServer) {
      REALMS_DATA_FETCHER.removeItem(pRealmsServer);
      this.realmsServers.remove(pRealmsServer);
